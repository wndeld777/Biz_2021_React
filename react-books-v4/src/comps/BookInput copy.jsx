...
function BookInput() {
   ...
   *  변수는 무조건 초기화 되어 버린다
   *
   * 초괴화 되는것을 방지하면서 현재 화면에서
   * 어떤 변수(public 변수)의 값을 유지하고 싶을때가 있다
   * 그럴때 reference 변수로 선언을 해주어야 한다
   */
  const nextId = useRef(0);
  /**
   * nextId 라는 변수를 만들고 입력창에 값이 입력되면 nextId 를 id 에 저장하기
   */
  ...
  // 도서정보를 배열에 저장하는데
  // 타이밍에 따라서 아직 UUID 가 미쳐 추가 되지 않은 상태에서 리스트에 추가되는 것을 방지하기 위하여
  // Promise 방식으로 함수를 실행한다
  ...
  const bookInsert = async () => {
    ...
    // 다음번 list 의 id 로 사용한다
    nextId.current++;

    /**
     * 리스트에 추가한 후 book state 를 초기화하여
     * 입력창에 입력된 내용을 삭제하기
     * 이 기능을 사용하려면
     * 반드시 input box value를 사용해야 한다
     * <input value={state} />
     *
     * <input defaultValue={state} /> 를 사용하면 기능이 효과 없음
     */
