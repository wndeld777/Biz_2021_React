 ...
 *
 * useState()
 * react 에서 "상태"를 생성하는 생성자 함수
 * 상태(state) : 값을 변화시킬 수 있는, 값이 변화되어야 하는 변수, 객체
 * 상태가 변하면 react는 화면을 다시 그린다
 * 변수나 객체의 값이 바뀌면(변경되면)
 * 		그에 맞춰서 화면이 다시 rendring된다
 * 만약 코드에서 변수나 객체를 수시로 바꾼다면?
 *	 	화면이 무작위로 무한대로 다시 rendring 될 것이다
 * 		컴퓨터의 fan 은 무한대로 발작을 할 것이다
 */
 ...
 *
 * HTML 에서 <input name="b_name" value="b_name"> 이라는 코드는
 ...
 * read only  가 되버리면 input 을 사용하는 의미가 없어진다
 * react 에서는 input box 에 change event 를 설정하고
...
const Write = () => {
  ...
  // 핸들러 함수
  /**
   * input[name="b_writer"] box 를 클릭하고
  ...
  /**
   * bbs.변수 타입으로 value 를 가지고 있는 input box 의
   * change event 핸들러
   */
  const onChangeHandle = (e) => {
    //	e.target 의 요소들을 분해(전개)하여 개별 변수로 선언
    // const name = e.target.name
    // const value = e.target.value
    // input box 의 이름과 키보드로 입력한 문자열을 추출할수 있다
    ...
    /**
     * 전개연산자를 사용하여 bbsVO 의 전체데이터를 복제하고
     * name 변수에 담긴 값과 같은 요소를 value 값으로 변경하여
     * 원래의 bbsVO 와 교체하라
     *
     * bbsVO[name] = value : 절대 사용하면 안되는 코드
     */
    setBBsVO({ ...bbsVO, [name]: value }); // set을 사용하여 이렇게 사용해야함
  };

  /**
   * <BBsView bbs={bbsVO} />
   * 1. import 된 BBsView 를 화면에 연결하라
