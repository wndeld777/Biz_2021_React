...
function RemList() {
   ...
   * 3번 활용(사용)한 것이다
   *
   * 일반적인 함수는 사용하기전에 만들고, 사용을 계속하고...
   * 이런식으로 진행이 되는데
   *
   * react 는 한번 rendering 된 화면은 특별한 값(상태)의 변화가 발생하면
   ...
   * 이전에 한번이라도 만들어진 경우는 그대로 재 사용 하고
   * 없는 경우에만 함수를 생성한다
   */
  ...
  const saveStorage = () => {
    ...
    if (rememberList.length > 0) {
       ...
       * return 1 : next 가 앞으로, pre 가 뒤로 이동
       * return -1 : pre 가 앞으로
       * return 0 : 그대로 있어
       *
  ...
  const onKeyDown = (e) => {
    ...
    if (e.keyCode === 13) {
       ...
       * const arCopy = [...ar] 새로운 arCopy 로 복제
       *
