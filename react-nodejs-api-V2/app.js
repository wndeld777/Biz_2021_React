...
const atlasURL =
  `mongodb+srv://` +
  `${USERID}` +
  `:${PASSWORD}@cluster0.fixun.mongodb.net/` +
...
// 작동되는 과정에 오류가 발생하면 console 에 오류메시지를 보여주기
dbConn.on("error", () => {
  console.error;
});
...
// API 서버를 구축할때는 필수적으로 CORS 를 허용해 주어야 한다
// Nodejs 에서는 cors import 하여 단순 설정만 하여도 CORS 를 허용할수 있다
// 무작정 CORS 를 허용하는 것은 매우 위험한 정책이다
